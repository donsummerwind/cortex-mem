...
flowchart TD
    ...
    C --> F[Intent Detection<br/>LLM or Heuristic Fallback]
    F --> F1[Query Rewriting<br/>max 150 chars]
    F --> F2[Keywords & Entities Extraction]
    
    F1 --> G[Generate Query Embedding]
    G --> H[Multi-Path Retrieval]
    
    H --> H1[Layered Search<br/>L0→L1→L2]
    H --> H2[Semantic Search<br/>Lower Threshold]
    H --> H3[Expanded Query Search]
    
    H1 --> I[Merge Results<br/>Dedup by URI]
    H2 --> I
    H3 --> I
    
    I --> J{Results Found?}
    J -->|Yes| K[Reranking Pipeline]
    J -->|No| L[Broad Semantic Search<br/>threshold=0.0]
    
    L --> M{Still Insufficient?}
    M -->|Yes| N[User-Scope Supplement<br/>cortex://user search]
    M -->|No| K
    N --> O[Lexical Fallback<br/>File Content Scan]
    O --> K
    
    K --> K1[Layer Score Adjustment<br/>L2+0.08, L1-0.04, L0-0.08]
    K1 --> K2[Keyword/Entity Hit Bonus]
    K2 --> K3[Intent-Path Bonus]
    K3 --> K4[Collection Summary Penalty]
    
    K4 --> P[Final Dedup & Truncate]
    P --> Q[Return Ranked Results]
```

#### 2.2.3 Multi-Path Retrieval Strategy

The service layer implements **multi-path retrieval** to maximize recall:

| Retrieval Path | Threshold | Purpose |
|----------------|-----------|---------|
| **Layered Search** | Adaptive (0.28-0.45) | Progressive L0→L1→L2 refinement |
| **Semantic Search** | 50% of input threshold | Broader candidate discovery |
| **Expanded Query** | 50% of input threshold | Synonym-related matches |
| **Broad Search** | 0.0 | Emergency fallback when others fail |
| **User-Scope Supplement** | 40% of input threshold | Ensures user-level memories included |
| **Lexical Fallback** | N/A | Keyword-based file scanning |

**Merging Strategy**:
- Results from all paths are merged by canonical URI
- Highest score wins for duplicate URIs
- Source tracking: `layered_vector`, `semantic_fallback`, `semantic_expansion`, `lexical_file`

#### 2.2.4 Query Profile Building

The service builds a `QueryProfile` for intent-specific optimization:

| Query Kind | Detection Keywords | Expansion Terms |
|------------|-------------------|-----------------|
| **Identity** | identity, transgender, nonbinary, gender, 身份 | identity, self, description, gender, background |
| **Temporal** | when, date, time, before, after, 什么时候 | date, time, event, timeline, timestamp |
| **Relationship** | relationship, married, partner, friend, 关系 | relationship, status, friend, partner, spouse |
| **Career** | career, education, job, work, study, 职业 | career, goal, work, education, study, aspiration |
| **Support** | support, help, motivate, encourage, 支持 | support, help, encourage, motivate, community |
| **Activity** | activit, hobby, sport, pottery, camping, 活动 | activities, hobby, interests, sports, recreation |

#### 2.2.5 Reranking Pipeline

After retrieval, results pass through a multi-factor reranking pipeline:

**Score Adjustments**:
```
bonus = base_source_bonus
      + (is_leaf ? 0.12 : -0.12)        // Leaf URIs get boost
      + keyword_hits * 0.03              // Query keyword matches
      + entity_hits * 0.05               // Named entity matches
      + collection_summary_penalty       // -0.14 for collection summaries
      + intent_path_bonus                // Intent-specific path boosts
```
...
#### 2.2.6 Layered Retrieval Algorithm
...
2. **L1 Layer (Overview)**: Deep exploration using `.overview.md` files (500-2000 tokens)
   ...
   - **Content**: Structured markdown with Summary, Core Topics, Key Points, Entities, Timeline Events, Activities
...
#### 2.2.7 Adaptive Query Processing

**Intent Detection**:
- **Entity Queries**: Short Chinese names (2-4 CJK characters) or short English proper nouns trigger specialized handling with threshold 0.28
- **Factual Queries**: Personal fact questions ("what did X research") use threshold 0.32
- **Temporal Queries**: Time-based constraints detected via keyword patterns, threshold 0.38

**Default Thresholds** (L0 layer):
| Intent Type | L0 Threshold |
|-------------|-------------|
| **EntityLookup** | 0.28 |
| **Factual** | 0.32 |
| **Search** | 0.35 |
...
1. **Progressive Threshold Reduction**: If L0 returns empty, reduce similarity threshold progressively
2. **Multi-Path Fallback**: Try semantic search with expanded query
3. **User-Scope Supplement**: Search user dimension specifically when global results insufficient
4. **Lexical Fallback**: File content keyword scanning when vector search returns <3 results
