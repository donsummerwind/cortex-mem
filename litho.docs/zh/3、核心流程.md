...
flowchart TD
    ...
    C --> F[意图检测<br/>LLM或启发式回退]
    F --> F1[查询重写<br/>最多150字符]
    F --> F2[关键词和实体提取]
    
    F1 --> G[生成查询嵌入]
    G --> H[多路检索]
    
    H --> H1[分层搜索<br/>L0→L1→L2]
    H --> H2[语义搜索<br/>降低阈值]
    H --> H3[扩展查询搜索]
    ...
    I --> J{找到结果?}
    J -->|是| K[重排序管道]
    J -->|否| L[宽泛语义搜索<br/>threshold=0.0]
    
    L --> M{仍然不足?}
    M -->|是| N[用户域补充<br/>cortex://user搜索]
    M -->|否| K
    N --> O[词法回退<br/>文件内容扫描]
    O --> K
    
    K --> K1[层级评分调整<br/>L2+0.08, L1-0.04, L0-0.08]
    K1 --> K2[关键词/实体命中奖励]
    K2 --> K3[意图路径奖励]
    K3 --> K4[集合摘要惩罚]
...
| 检索路径 | 阈值 | 目的 |
|----------------|-----------|---------|
| **分层搜索** | 自适应(0.28-0.45) | 渐进L0→L1→L2精炼 |
| **语义搜索** | 输入阈值的50% | 更广泛的候选发现 |
| **扩展查询** | 输入阈值的50% | 同义词相关匹配 |
| **宽泛搜索** | 0.0 | 其他路径失败时的紧急回退 |
| **用户域补充** | 输入阈值的40% | 确保用户级记忆被包含 |
| **词法回退** | 无 | 基于关键词的文件扫描 |

**合并策略**：
- 所有路径的结果按规范化URI合并
- 重复URI取最高分
- 来源追踪：`layered_vector`、`semantic_fallback`、`semantic_expansion`、`lexical_file`

#### 2.2.4 查询画像构建

服务构建`QueryProfile`用于意图特定优化：
...
```
bonus = 基础来源奖励
      + (is_leaf ? 0.12 : -0.12)        // 叶子URI获得提升
      + keyword_hits * 0.03              // 查询关键词匹配
      + entity_hits * 0.05               // 命名实体匹配
      + collection_summary_penalty       // 集合摘要惩罚 -0.14
      + intent_path_bonus                // 意图特定路径奖励
```
...
2. **L1层（概览）**: 使用`.overview.md`文件（500-2000 tokens）的深度探索
   - **目的**: 决策的上下文精炼
   - **权重**: 最终评分的30%
   - **内容**: 带摘要、核心主题、关键点、实体、时间线事件、活动的结构化markdown
...
**意图检测**：
- **实体查询**: 短中文名（2-4个CJK字符）或短英文专有名词触发专门处理，阈值为0.28
- **事实查询**: 个人事实问题（"X研究什么"）使用阈值0.32
- **时间查询**: 通过关键词模式检测的基于时间的约束，阈值0.38
...
**降级策略**：
1. **渐进阈值降低**: 如果L0返回空，逐步降低相似度阈值
2. **多路回退**: 尝试使用扩展查询的语义搜索
3. **用户域补充**: 当全局结果不足时专门搜索用户维度
4. **词法回退**: 当向量搜索返回<3个结果时进行文件内容关键词扫描
